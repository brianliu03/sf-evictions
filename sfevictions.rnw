...
<<eval=FALSE>>=
# Calculate the expected proportion for each eviction type
calc_proportions <- function(data) {
  data %>%
    count(eviction_type) %>%
    mutate(prop = n / sum_evictions_09_23) %>%
    arrange(eviction_type)
}

# Pass to function then bind data_id which is the period
e_prop <- lapply(evictions_09_23, calc_proportions)
e_prop <- bind_rows(e_prop_tp, .id = "data_id")

expected_prop <- sum(e_prop_tp %>% pull(prop)) / 48
@
...
get_bs <- function(size, type, data) {
  # 4 by size matrix, 4 rows for each time period
...
bs_matrices <- list()
for (i in 1:12) {
  bs_matrices[[i]] <- get_bs(size, eviction_type[i], e_09_23)
}
